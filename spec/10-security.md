...
> **Revision (XV-2):** Binding — every human approval of an R4 gate decision MUST be executed as (a) a hardware-security-key signature over the decision digest (the §6.3 treasury co-sign mechanism, extended to all R4 gates including G-16), and (b) out-of-band confirmation on a pre-registered second channel whenever the approval request, its urgency claim, or any material input arrived through any channel other than the Kernel-rendered gate queue. Step-up within a session is never sufficient for R4. See Part XV (`15-critique-and-revisions.md`) §4.
...
> **Revision (XV-2):** Binding — key-recovery ceremonies (§6.4) and degraded-mode manual treasury operations (§12.1) MUST operate under two-person integrity with session recording and mandatory post-hoc ARC review; the degraded mode inherits the dual-control rules, not merely the access paths. A recovery ceremony convened outside the documented calendar or without a declared triggering incident is itself a Sev-2. See Part XV (`15-critique-and-revisions.md`) §4.
