...
> **Revision (XV-15):** The `RISK-QUANT` factor model MUST carry **"frontier-AI price/capability"** as a standing named factor from inception, with an IC-ratified explicit exposure limit alongside the §8.5 limits. Every G-07 and G-08 DR MUST include the AI-cost/capability stress scenario (RISK-ECON-03 stress-priced economics plus a capability-plateau case) as a mandatory sensitivity, and the §15 capital-at-risk dashboard reports aggregate loading on this factor to the IC and ARC. See Part XV (`15-critique-and-revisions.md`) §4.
...
> **Revision (XV-8):** Before the first G-08 tranche clears, the CFO and `FIN-DIR` MUST present, and the IC MUST ratify as an R4 DR: (1) the capital-structure decision (evergreen HoldCo vs. fund vs. hybrid, with GC — ownership instruments, distribution/recycle waterfall, and confirmation that no instrument grants any capital provider a bypass of G-16); (2) the platform-cost recovery mechanism, shown on the §6.2 forecast to repay the platform; (3) a portfolio returns model benchmarked against both the vintage-matched VC median (Part I M-4) and published venture-studio base rates, with the gap to each stated. G-08 is closed until this ratification exists (milestone M6b, Part XIV §10). See Part XV (`15-critique-and-revisions.md`) §4.
