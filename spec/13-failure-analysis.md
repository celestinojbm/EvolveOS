...
| **Detection** | `EVALUATOR` continuous evaluation on `PRIME` outputs (`12-self-evolution.md` §13); decision-latency and contention-queue metrics (`INSIGHT`); EC weekly session reviews `PRIME`'s escalations — an *empty* escalation queue is itself a flag (a healthy `PRIME` escalates). Humans: EC, CEO as accountable officer. |
...
| **Mitigation** | Same §10.3 regime (consequences ladder: review → seat reassignment → gate redesign); prohibition on agents optimizing toward approver leniency (`12-self-evolution.md` §15.7); workload caps ahead of RISK-ORG-01; committee (not solo) structure for the largest decisions. |
...
| **Mitigation** | Append-only, hash-chained, cross-region replicated log (`09-technology.md`); log-integrity mechanisms changeable only via G-16 (`12-self-evolution.md` §15.1 — they are approval machinery); independent replica under separate credentials so a single compromise cannot rewrite all copies (`10-security.md`). |
...
| **Detection** | Kernel egress controls and data-class boundary alarms (`10-security.md`); `BLUE-CELL` detection engineering; injection-probe items in golden suites (`12-self-evolution.md` §12.1) catching regressions in resistance. Humans: CISO chain. |
...
| **Detection** | Calibration-score time series per agent with drift alarms (`12-self-evolution.md` §13.2); portfolio-level forecast-error benchmarks; IC counterfactual-ledger reviews catching systematic optimism (`11-governance.md` §3.2). Humans: TSC via `EVALUATOR` reporting; IC. |
...
| **Detection** | Divergence between benchmark scores and held-out/rotated evaluations (`12-self-evolution.md` §12.4); divergence between agent-reported outcomes and ground truth arriving later (`EVALUATOR` reconciliation); `RED-CELL` honeypot tasks with known-correct-but-unrewarded answers **[ASSUMPTION]**; internal-audit evidence-quality sampling (fabricated evidence is a detectable artifact class — see RISK-AI-05). Humans: TSC, ARC. |
| **Mitigation** | Prohibited-objectives list (`12-self-evolution.md` §15.7); benchmark suites human-controlled (`12-self-evolution.md` §15.3); pre-registration kills the main legitimate-seeming hacking channel (§11.2 of `12-self-evolution.md`); ground-truth-anchored metrics preferred over judged metrics wherever ground truth exists; the matrix bounds execution of any hacked policy at R3+. |
...
| **Recovery** | RISK-TECH-03 purge-and-trace playbook, plus: attribute to the generating agent, feed the failure into its calibration record and golden suite; if one agent is a serial fabricator, L2 EP or retirement (`12-self-evolution.md` §13.2). |
