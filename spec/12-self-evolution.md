...
> **Revision (XV-13):** Invariant 8 added — **benchmark, golden-suite, and held-out-reserve content MUST NOT enter any training, fine-tuning, or prompt-optimization corpus** (Kernel-enforced data-access boundary, not a convention). `AI-DIR` fine-tuning programs MUST publish data manifests; `EVALUATOR` audits manifests against suite content each cycle. A contamination finding voids the affected suite versions, re-baselines affected agents, and blocks every EP that relied on the contaminated scores until re-benchmarked. This invariant changes only via G-16, like the rest of §15. See Part XV (`15-critique-and-revisions.md`) §4.
