...
Since issue #10 the DR is **filed immutably** before it is approved, so `objectDigest` is the DR's **stored** content digest (computed by `dr.ts`, the single canonicalization owner — see [DECISION_RECORDS](DECISION_RECORDS.md)); the gate later loads the filed DR by id and checks the approval binds to that stored digest.
