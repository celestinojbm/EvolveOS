...
- **Agent invocation.** Phase 0 has **no productive agent runtime**. The **only** productive boundary is `runGuardedAgentInvocation(client, fn)`: the boundary check **and** the callback run in **one transaction holding the advisory lock**, so a stop cannot commit between the check and the callback. If the system is stopped (or corrupt) `fn` is never called; any future credential resolution / model call / tool / side effect MUST happen **inside** `fn`. `assertAgentInvocationAllowed` is a **check-only** point read and is **not** a sufficient boundary — the pattern `await assertAgentInvocationAllowed(c); await invoke()` has a TOCTOU window (the lock is released before `invoke`) and is forbidden. `check-stop-writer` prevents a second boundary from being defined **and** forbids any reference to the check-only `assertAgentInvocationAllowed` outside `stop.ts` (so the check-then-invoke shape can't be written); it does not, and does not claim to, prove a given call site holds the lock across its callback.
