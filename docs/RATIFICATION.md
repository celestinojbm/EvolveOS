...
The whole evaluation runs inside **one transaction at the default `READ COMMITTED` isolation with the event-chain advisory lock taken FIRST**. This is deliberately *not* `REPEATABLE READ`: under `REPEATABLE READ` the data snapshot is fixed at the first statement, so if that first statement is the lock acquisition, the snapshot can be pinned to an instant *before* the lock is granted — a revocation that committed while we waited would be invisible (a stale-snapshot `true`). With `READ COMMITTED` + lock-first, every audited mutation (role grant/revoke, signature recording, session start/end) takes that same lock, so none can interleave while we hold it, and each of our reads sees the state committed *after* the lock was granted. It is totally ordered against those mutations; on any genuine SQL/connection error it rolls back and propagates (malformed event *data* is treated as invalid evidence, never raised); it never returns `true` from a partial state. Any doubt → `false`. A fresh database is always `false`.

**Timestamps are parsed safely.** `event.timestamp` is untrusted text (the `events.timestamp` column is `TEXT`). `parseEventTimestamp` accepts only a strict ISO-8601/RFC3339 instant with *real* calendar fields (no `2026-02-30` roll-over) and passes the resulting `Date` to Postgres as a bound parameter — never interpolated as `$n::timestamptz`. A malformed timestamp makes that event invalid evidence; it never throws and never blocks the real signature. Both the idempotency search and the flag evaluation use the **same** full-evidence primitive (`signatureEvidenceIsFullyValid` = structural check + historical session validity), so a structurally correct event with a missing/foreign/out-of-window/invalid-time session can neither count nor be mistaken for an existing signature.
