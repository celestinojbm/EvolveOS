...
- **Records are schemas, not stores (except the DR since issue #10).** The Knowledge Item and Task Contract schemas define shape only. The event schema gained its store in issue #6; the **Decision Record schema was extended to `1.0.0`** (options, chosen option, dissent, amendment link) and given an immutable store + validation + brief renderer in issue #10 — see [Decision Records](DECISION_RECORDS.md).
