...
- **Column↔document binding CHECKs**, all three **NULL-safe** via `IS NOT DISTINCT FROM`: `document_json->>'id'` vs `id`, `->>'schema_version'` vs `schema_version`, `->>'amends_dr_id'` vs `amends_dr_id`. NULL-safety is the point — with a plain `=`, a `document_json` *missing* the key would make the comparison NULL, and a NULL CHECK **passes**, silently admitting the very row the constraint exists to reject. A projection that disagrees with (or omits) the row identity/version/amendment-link is unrepresentable even by direct INSERT.
...
| `fileDecisionRecord({ document, filedBy, year? })` | mint id, stamp `schema_version`, validate, emit one `decision_record.filed`, insert — atomic; request captured before the first `await`, advisory lock first; returns a **deep-frozen** document (the result represents archived bytes — it cannot be locally mutated into something that only *looks* archived) |
| `fileDecisionRecordAmendment({ amendsDrId, document, filedBy, year? })` | a new linked DR + `decision_record.amended`; result equally deep-frozen |
...
| `renderDecisionBrief(record: StoredDecisionRecord)` | deterministic ≤2-page markdown brief (§8.2), **cryptographically verified** before rendering (below) |
...
**The brief is bound to the archived content.** `renderDecisionBrief` takes a full `StoredDecisionRecord` and, while staying pure (no database access), verifies it **cryptographically** before rendering a single line: the document is re-snapshotted (`snapshotDecisionRecordContent` — deep clone, full validation, canonicalization, SHA-256) and must reproduce the record's `digest` and `canonicalJson` exactly, and the record's `id` / `amendsDrId` must match the document. Any mismatch — a tampered decision, option, or dissent entry paired with the original digest; a fake digest or canonical string; a mislabeled id or amendment link — throws **`DecisionBriefIntegrityError`**. Rendering then uses the *verified snapshot clone*, never the caller's object, so the brief can never show content that does not correspond to the digest it prints in its drill-down.

It produces deterministic markdown in the exact order: **1** the ask (one sentence + reversibility + gate; §8.2 also names the envelope delta, but the Phase 0 DR schema has no envelope field — envelopes are unratified per ADR-006 and v0 authorizes no spend — so no delta exists to show and none is invented), **2** options table (chosen marked, uncertainty shown, no invented scores — and always real for R3/R4, whose options cannot be filed without a distribution). Part VII admits multiple distribution representations, so the renderer never hides one it doesn't recognize: if none of the known fields (`primary_metric`, `representation`, `quantiles`) are present, the *predicted outcome* cell shows the distribution's deterministic canonical JSON (markdown-escaped), and the *uncertainty* cell falls back `epistemic_share` → `representation` → the explicit note *"structured distribution — see predicted outcome"* — a valid, non-empty distribution never renders as "—". **3** top three risks, each stating its mitigation **or its absence flatly** — the Phase 0 DR keeps `risks: string[]` with no separate mitigation field, so every shown risk carries the explicit line *"Mitigation: not separately recorded in the Phase 0 DR."* (declared honestly, never invented; the rest are pointed to, not hidden), **4** dissent **verbatim**, **5** kill criteria + rollback, **6** drill-down (id + digest + evidence + amend link). See [`docs/templates/dr-template.md`](templates/dr-template.md) for the shape and a rendered example.
...
**Guarantees:** invalid DRs are rejected with field-level errors; a filed DR is immutable (DB-enforced) and can only be revised by a linked amendment; one canonicalization/digest shared with the gate system (no second definition); reads are a full trust boundary (digest, canonical bytes, JSONB, re-validation, column↔document bindings, and the filing event itself) that rejects corruption instead of returning dubious bytes; both reads **and filing results** return deep-frozen documents; a gate pass cannot cite an unfiled DR even at the DB layer (`gate_passes.dr_id` FK); R3/R4 DRs always carry per-option uncertainty and ≥1 risk; the brief is **cryptographically bound to the archived content** (`DecisionBriefIntegrityError` on any mismatch), renders every mandatory section deterministically — each shown risk stating mitigation or its absence, unrecognized distributions rendered canonically instead of hidden — and refuses to ship over-length rather than truncate.
