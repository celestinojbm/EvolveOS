...
0. **Whole-request snapshot before the first `await`.** Every public pass function (`passG01CreateVenture`, `passPipelineGate`, `passStandingGate`, and the `passGate` dispatcher) reads **each request field exactly once, synchronously**, into a frozen local snapshot *before* it opens the transaction, then never touches the caller's `input` again — validation, mutation, event, projection, and the returned result all read the snapshot. This closes the mutable-request TOCTOU for the **whole request**, not just the DR: mutating `ventureId`, `approvalEventId`, `actor`, `gateId`, `name`, `opportunityRef`, `year`, `subjectType`, or `subjectId` after the call cannot change which venture advances, which approval is checked, or what is recorded. Required auditable identifiers are validated non-empty and captured **verbatim** (an id is never silently rewritten). Tests prove immunity: a decoy venture B stays put while A advances, a bogus `approvalEventId` set after the call is ignored, and the recorded actor/subject/opportunity-ref are the originals.
1. **Decision Record — canonical immutable snapshot.** As part of that same pre-`await` snapshot, every pass runs `snapshotDecisionRecord`: canonical deterministic JSON (key-sorted), a **deep clone** (the caller's object is never read again — later mutations cannot affect the pass), schema validation of the clone against [`schemas/decision-record.schema.json`](../schemas/decision-record.schema.json) (ajv, draft 2020-12), semantics (`gate_id` equals the requested gate, `status: approved`, non-empty `proposer`, non-null/non-empty `approver`, `proposer ≠ approver`), the **canonical reversibility class** (`dr.reversibility_class` must equal the gate's `reversibility_class` from `gates.json` — e.g. *"gate G-05 requires R3, DR declares R2"* is a specific error), and the **SHA-256 content digest** of the canonical JSON.
...
**Guarantees:** no gate pass without a schema-valid approved DR, pre-registered kill criteria (where mandatory), real linked human approval by a live approver bound to the exact DR content (SHA-256-hex digest, format-validated), and role separation; exactly one `gate_passed` + one projection row per pass, atomic with the venture effect; a DR never executes twice; the whole request is snapshotted before the first `await`, so post-call mutation of any field cannot change the pass; the gate registry and its lists are immutable at runtime (no re-classification, add, or removal); no production bypass of the gated paths (CI-enforced).
