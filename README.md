...
| [Founding Ratification](docs/RATIFICATION.md) | The signable [Ratification Pack](docs/RATIFICATION_PACK.md) + the purely-derived `real_money` flag: exact-byte digest, human-only signatures, no override, and why `true` still moves no money — Phase 0 issue [#11](https://github.com/celestinojbm/EvolveOS/issues/11) |
| [G-00 Manual Stop](docs/G00_STOP.md) | Emergency stop: any authorized human halts all gate and agent-invocation paths in one call (reason optional); approver + logged rationale restarts; stop asymmetry, singleton state, events, fail-closed — Phase 0 issue [#12](https://github.com/celestinojbm/EvolveOS/issues/12) |
| [Audit Conventions](docs/AUDIT_CONVENTIONS.md) | The Phase 0 event taxonomy (every type's actor/object/payload contract) + the `verify-log` CLI: re-verify the whole hash chain and every convention, and extract a human audit trail by date range or venture — Phase 0 issue [#13](https://github.com/celestinojbm/EvolveOS/issues/13) |
