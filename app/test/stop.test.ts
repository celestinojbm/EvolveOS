 ...
 *
 * Real PostgreSQL. Covers: genesis running state, stop authorization (any
 * authorized human; every rejection), atomicity, idempotence, the central gate
 * guard on every public route, the agent-invocation boundary (fake local
 * callback — no real agents/credentials/models), restart (approver + rationale),
 * deterministic stop-vs-gate / stop-vs-invocation / concurrency ordering (no
 * sleeps), fail-closed corruption, and the singleton/constraint integrity.
 *
 * The system_stop_state singleton is global to the DB; vitest runs files
 * serially (fileParallelism:false) and afterEach restores the genesis RUNNING
 * state, so no leaked stop can affect another file.
 */
...
/** Create a registered user with the given roles and an active session. */
async function makeHuman(client: pg.Client, tag: string, roles: Role[]): Promise<Human> {
  const actorId = `usr-${runId}-${tag}-${humanSeq++}`;
...
/** Restore the genesis RUNNING state (re-inserts if the row was deleted). */
async function resetRunning(client: pg.Client): Promise<void> {
  await client.query(
    `INSERT INTO system_stop_state (singleton, is_stopped, generation)
       VALUES (TRUE, FALSE, 0)
     ON CONFLICT (singleton)
       DO UPDATE SET is_stopped = FALSE, generation = 0, current_event_id = NULL,
                     actor_id = NULL, reason = NULL, updated_at = now()`,
  );
}
...
describe("G-00 stop — initial state, authorization, atomicity, idempotence", () => {
  let client: pg.Client;
  beforeAll(async () => {
    client = new pg.Client({ connectionString: DATABASE_URL });
    await client.connect();
  });
  afterEach(async () => resetRunning(client));
  afterAll(async () => {
    await resetRunning(client);
    await client.end();
  });

  it("a fresh/genesis DB is RUNNING with no fabricated stop event; guards allow", async () => {
    ...
    await expect(assertSystemRunning(client)).resolves.toBeUndefined();
    let ran = 0;
  ...
  it("an unregistered user, bad/ended/foreign session, or no-role user cannot stop", async () => {
    ...
    // Registered user with an active session but NO active role.
    const noRole = `usr-${runId}-norole`;
  ...
  it("reason is optional: absent and whitespace store null; a real reason is preserved", async () => {
    ...
    await stop(client, h1); // no reason
  ...
  it("a stop emits exactly one event, atomic with the projection, chain intact", async () => {
    ...
    // The event carries generation, reason, session, and the right object.
    ...
    expect(rows[0].payload.generation).toBe(1);
  ...
  it("a projection-write failure rolls back the stop event (no partial effect)", async () => {
    ...
    expect(await countEvents(client, STOP_EVENT_TYPE)).toBe(before); // event rolled back
    expect((await getSystemStopState(client)).isStopped).toBe(false);
  });
  ...
  it("stopping when already stopped is idempotent: same event, no generation bump", async () => {
    ...
    expect(await countEvents(client, STOP_EVENT_TYPE)).toBe(before); // no new event
    expect((await getSystemStopState(client)).generation).toBe(1); // unchanged
  });
});
...
describe("G-00 stop — restart (approver + rationale)", () => {
  ...
  it("an approver with a non-empty rationale can restart; the event records it and the released stop", async () => {
  ...
  it("restart without an approver role, or by a viewer/operator, is rejected", async () => {
    const stopper = await makeHuman(client, "rn-stop", ["operator"]);
    await stop(client, stopper);
    const op = await makeHuman(client, "rn-op", ["operator"]);
    await expect(restart(client, op)).rejects.toThrow(RestartRequiresApproverError);
    const viewer = await makeHuman(client, "rn-view", ["viewer"]);
    await expect(restart(client, viewer)).rejects.toThrow(RestartRequiresApproverError);
    expect((await getSystemStopState(client)).isStopped).toBe(true);
  });

  it("restart with an absent/whitespace rationale is rejected", async () => {
  ...
  it("restart with an invalid session is rejected", async () => {
    const stopper = await makeHuman(client, "rs-stop", ["operator"]);
    await stop(client, stopper);
    const approver = await makeHuman(client, "rs-appr", ["approver"]);
  ...
  it("restart when the system is already running is rejected with no event", async () => {
  ...
  it("the restarter need not be the human who stopped", async () => {
    const stopper = await makeHuman(client, "diff-stop", ["viewer"]);
    await stop(client, stopper);
    const approver = await makeHuman(client, "diff-appr", ["approver"]);
...
describe("G-00 stop — halts every gate route and the agent boundary", () => {
  ...
  it("while stopped, a pipeline gate is refused; after restart it works", async () => {
    const vid = await ventureTo(client, actors, "research"); // built while running
    await stop(client, stopper);
    ...
    expect(await countEvents(client, "gate_passed")).toBe(beforeGate);
    // Restart, then the same route succeeds (fresh DR).
  ...
  it("the agent boundary: running runs the callback once; stopped never runs it; restart re-allows", async () => {
    ...
    expect(ran).toBe(1); // callback did NOT run while stopped
    await restart(client, approver, "resume");
    await runGuardedAgentInvocation(client, () => { ran++; });
    expect(ran).toBe(2);
  });
...
describe("G-00 stop — fail closed & integrity", () => {
  let client: pg.Client;
  beforeAll(async () => {
    client = new pg.Client({ connectionString: DATABASE_URL });
    await client.connect();
  ...
  it("a missing singleton row fails closed (no permission), and the callback never runs", async () => {
    await client.query("DELETE FROM system_stop_state");
    await expect(assertSystemRunning(client)).rejects.toThrow(StopStateCorruptError);
    let ran = 0;
    await expect(runGuardedAgentInvocation(client, () => { ran++; })).rejects.toThrow(StopStateCorruptError);
    expect(ran).toBe(0);
    await resetRunning(client); // restore for other tests
  });
  ...
  it("a second singleton row is impossible", async () => {
    ...
    ).rejects.toThrow(); // PK conflict on the constant singleton
  });

  it("an incoherent state is rejected by the coherence constraint", async () => {
    // generation >= 1 with no event/actor violates the coherence CHECK.
    await expect(
      client.query("UPDATE system_stop_state SET is_stopped = TRUE, generation = 1 WHERE singleton = TRUE"),
  ...
  it("stop.ts reads no environment override", () => {
    const src = readFileSync(join(fileURLToPath(new URL(".", import.meta.url)), "..", "src", "lib", "stop.ts"), "utf8");
...
describe("G-00 stop — deterministic concurrency & ordering (Postgres)", () => {
  let a: pg.Client;
  let b: pg.Client;
  let h: pg.Client; // lock holder
  let obs: pg.Client; // pg_locks observer
  beforeAll(async () => {
  ...
  afterEach(async () => resetRunning(a));
  afterAll(async () => {
    await resetRunning(a);
  ...
  it("two concurrent stops collapse to exactly one event", async () => {
    ...
    expect(await countEvents(a, STOP_EVENT_TYPE)).toBe(before + 1); // exactly one effective
    expect((await getSystemStopState(a)).generation).toBe(1);
  });
  ...
  it("two concurrent restarts collapse to exactly one effective release", async () => {
    ...
    expect(await countEvents(a, RESTART_EVENT_TYPE)).toBe(before + 1); // one release event
    expect(results.filter((r) => r.status === "fulfilled").length).toBe(1);
    expect(results.filter((r) => r.status === "rejected").length).toBe(1); // the loser: "not stopped"
    expect((await getSystemStopState(a)).isStopped).toBe(false);
  });

  it("stop that acquires the lock first → a queued gate pass then refuses", async () => {
    const actors = await setupActors(a, "race-svg");
    const vid = await ventureTo(a, actors, "research"); // running
    ...
    // Holder grabs the event-chain lock so both contenders must queue.
    await h.query("BEGIN");
    ...
    // Stop queues FIRST, then the gate — advisory locks are granted in wait order.
    ...
    await h.query("COMMIT"); // release; stop runs, then the gate
    await expect(stopP).resolves.toMatchObject({ idempotent: false });
    await expect(gateP).rejects.toThrow(SystemStoppedError);
  });

  it("gate pass that acquires the lock first completes; the stop lands right after; the next pass refuses", async () => {
    const actors = await setupActors(a, "race-gvs");
    const vid = await ventureTo(a, actors, "research");
    const stopper = await makeHuman(a, "race-gvs-stop", ["operator"]);
    const filed = await fileDR(a, actors, { gateId: "G-02" });
    const appr = await approveDR(a, actors, filed);
    ...
    // Gate queues FIRST, then the stop.
    const gateP = passPipelineGate(a, { gateId: "G-02", ventureId: vid, decisionRecordId: filed.drId, approvalEventId: appr, actor: actors.approver });
    await waitForLockWaiter(obs, gatePid);
    ...
    await expect(gateP).resolves.toMatchObject({ gateId: "G-02" }); // completed coherently
    await expect(stopP).resolves.toMatchObject({ idempotent: false }); // stop landed right after

    // A subsequent pass now refuses.
    const filed2 = await fileDR(a, actors, { gateId: "G-03" });
    const appr2 = await approveDR(a, actors, filed2);
    await expect(
      passPipelineGate(a, { gateId: "G-03", ventureId: vid, decisionRecordId: filed2.drId, approvalEventId: appr2, actor: actors.approver }),
    ).rejects.toThrow(SystemStoppedError);
  });
});
